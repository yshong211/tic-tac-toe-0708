...
export default class GetDim extends React.Component {
    ...
    handleChange(event) {
      this.setState({rows: event.target.value, winning: event.target.value});
    }
    /*
    handleChange2(event) {
        this.setState({winning: event.target.value});
      }
      */
    how_many(rows) {
        let how = [];
    ...
    render() {
      ...
      <div className = 'submission'>
        <form>
          ...
          <br />
          <label>
            <select value={this.state.winning} onChange={this.handleChange}>

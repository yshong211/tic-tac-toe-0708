...
export default class Game extends React.Component {
    ...
    vert(squares, row_length){
      ...
      let o = 0;
      for(let i = 0; i < row_length; i++){
        for(let j = i; j < row_length**2; j+=row_length){
          if(squares[j]==='X'){
            x++;
          }
          if(squares[j]==='O'){
    ...
    calculateWinner(squares, rows){
      ...
      let x = 0;
      let y = 0;
      if(this.hor(squares, rows,count,x,y) === 'O' || this.hor(squares, rows,count,x,y) === 'X'){
        return this.hor(squares, rows,count,x,y);
      }
      
      else if(this.vert(squares, rows,count,x,y) === 'O' || this.vert(squares, rows,count,x,y) === 'X'){
        return this.vert(squares, rows,count,x,y);
      }

      else if(this.cross(squares, rows,count,x,y) === 'O' || this.cross(squares, rows,count,x,y) === 'X'){
        return this.cross(squares, rows,count,x,y);
      }
      return null;
    }
    handleClick(i) {

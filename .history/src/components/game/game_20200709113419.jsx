...
export default class Game extends React.Component {
  ...
  cross(squares, row_length, count, x, o) {
    ...
    for (let i = row_length - 1; i < row_length ** 2 - 1; i += row_length - 1) {
      if (squares[i] === 'X') {
        x++;
      }
      if (squares[i] === 'O') {
  ...
  calculateWinner(squares, rows) {
    ...
    if (
      this.hor(squares, rows, board_rows, count, x, o) === 'O' ||
      this.hor(squares, rows, board_rows,count, x, o) === 'X'
    ) {
      return this.hor(squares, rows, board_rows,count, x, o);
    } else if (
      this.vert(squares, rows, board_rows,count, x, o) === 'O' ||
      this.vert(squares, rows, board_rows,count, x, o) === 'X'
    ) {

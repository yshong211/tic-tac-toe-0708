...
export default class Game extends React.Component {
  ...
  rl_cross(squares, row_length, count, x, o) {
    let track = 1;
    for (let k = 0; k < row_length ** 2 - row_length * 2; k++) {
      for (let i = k; i < row_length ** 2; i += row_length + 1) {
        if (squares[i] === 'X') {
          x++;
          o = 0;
          count++;
  ...
  calculateWinner(squares, rows) {
    ...
    if (this.rl_cross(squares, rows, count, x, o)) {
      return this.rl_cross(squares, rows, count, x, o);
    }
    return null;
  }
